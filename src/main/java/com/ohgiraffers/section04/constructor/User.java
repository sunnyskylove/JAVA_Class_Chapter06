...
public class User {
    ...
    *   2. 생성자 메소드는 반환형(자료형, void / return 하는~)을 작성하지 않는다.
    *      (작성하는 경우 생성자가 아닌 일반 메소드로 인식한다.)
    * */

    /* 필기.
    *   생성자의 종류
    *   1. 기본 생성자(매개변수 없이 생성자)
    *   2. 매개변수 있는 생성자
    *    기본 생성자는 자바 complier 가 자동으로 추가해주는 구문이지만, 명시적으로 작성할 수 있다.
    *    매개변수가 있는 생성자가 한 개라도 존재하는 경우에는 기본생성자를 자동으로 추가해주지 않기 때문에
    *    기본 생성자가 필요한 경우에는 반드시 명시적으로 작성해 주어야 한다.(기본생성자 없으면 오류남)
    * */


    /* 목차. 1. 기본생성자 (default constructor) */
    public User(){                      // 메소드와 비슷하게 쓰이지만, 다른 존재이다!! - 기본??
    ...
    public User(String id, String pwd, String name){                //- 매개변수 초기화??

        /* 필기. 1. 매개변수 있는 생성자의 주 목적은 인스턴스 생성 시점에 매개변수로 전달
        *       받은 값을 이용해서 필드를 초기화 하는 목적이다.
        * */
        this.id = id;                       // 값을 전달받아서 초기화 해주는 것임!!
        this.pwd = pwd;
        this.name = name;
...
//        this.enrollDate = enrollDate;
        ...
        *   this() 는 가장 첫 줄에 선언해야 하며, 그렇지 않는 경우 Complie Error가 발생한다.
        * */
